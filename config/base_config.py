...
# Basic configuration
PLATFORM = os.getenv("PLATFORM", "xhs")  # Platform, xhs | dy | ks | bili | wb | tieba | zhihu
...
KEYWORDS = os.getenv("KEYWORDS", "编程副业,编程兼职")  # Keyword search configuration, separated by English commas
LOGIN_TYPE = os.getenv("LOGIN_TYPE", "qrcode")  # qrcode or phone or cookie
COOKIES = os.getenv("COOKIES", "")
CRAWLER_TYPE = (
    os.getenv("CRAWLER_TYPE", "search")  # Crawling type, search (keyword search) | detail (post details) | creator (creator homepage data)
)
...
# 浏览器启动超时时间（秒）
BROWSER_LAUNCH_TIMEOUT = _env_int("BROWSER_LAUNCH_TIMEOUT", 60)
...
SAVE_DATA_OPTION = os.getenv("SAVE_DATA_OPTION", "jsonl")  # csv or db or json or jsonl or sqlite or excel or postgres

# Data saving path, if not specified by default, it will be saved to the data folder.
SAVE_DATA_PATH = os.getenv("SAVE_DATA_PATH", "")

# Browser file configuration cached by the user's browser
USER_DATA_DIR = os.getenv("USER_DATA_DIR", "%s_user_data_dir")  # %s will be replaced by platform name
...
CRAWLER_MAX_COMMENTS_COUNT_SINGLENOTES = _env_int("CRAWLER_MAX_COMMENTS_COUNT_SINGLENOTES", 10)
